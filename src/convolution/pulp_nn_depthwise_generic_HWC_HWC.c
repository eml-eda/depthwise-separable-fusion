...
void __attribute__ ((noinline)) pulp_nn_depthwise_generic_HWC_HWC(
  ...
  // Il buffer Im2Col creato e' comunque in CHW 
  ...
  for(int i_out_ch = start_channel; i_out_ch < stop_channel; i_out_ch++)
  {
    ...
    if(padding_x_left > 0)
    {
      do
      {
        ...
        uint8_t *base_ptr = Im_in + i_out_ch; // Puntatore al primo elemento dei canali in HWC
        do
        {
          ...
          {
            *(pBuffer++) = *(base_ptr + 0);
            *(pBuffer++) = *(base_ptr + (ch_im_in));
            *(pBuffer++) = *(base_ptr + (ch_im_in<<1));
            *(pBuffer++) = *(base_ptr + (ch_im_in<<1) + ch_im_in);
            idx+=4;
            i++;
          }while(i<dim_kernel_x_size_padded);
          pBuffer-=(dim_incr + const1);
          // Qua passa alla riga dopo, 
          // base_ptr+=dim_im_in_x;
          base_ptr+=(dim_im_in_x * ch_im_in);
          i_buff_y++;
    ...
    do
    {
      ...
      // i_ch_im punta al canale CHW giusto.
      // int base_ptr = Im_in + i_ch_im +  (i_out_x * stride_x) - padding_x_left;
      uint8_t *base_ptr = Im_in + i_out_ch +  (i_out_x * stride_x * ch_im_in) - padding_x_left * ch_im_in;
      do
      {
        int idx = 0;
        for (int i=0; i<dim_kernel_x_size_padded; i++)
        {
          // HWC input che scrive in CHW
          *pBuffer = *(base_ptr + idx);
          pBuffer++;
          idx+= ch_im_in;
            asm volatile("":::"memory");
          *pBuffer = *(base_ptr + idx);
    ...
    for (i_out_x; i_out_x < dim_im_out_x; i_out_x++)
    {
      ...
      do
      {
        ...
        do
        {
          *(uint8_t *) pBuffer = ((uint8_t) *(base_ptr + idx));
          pBuffer++;
          idx+= ch_im_in;
            asm volatile("":::"memory");
          *(uint8_t *) pBuffer = ((uint8_t) *(base_ptr + idx));
