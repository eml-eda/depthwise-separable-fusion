...
void __attribute__ ((noinline)) pulp_nn_dw_pw_HWC_HWC_HWC_rows_base(
  ...
  uint16_t intermediate_rows_padded;
  ...
  pOut += intermediate_rows* dim_im_out_x * ch_im_out;
  pIn += (stride_y * intermediate_rows - padding_y_top) * dim_im_in_x * ch_im_in; 
  ...
  for(current_out_row = intermediate_rows; current_out_row <= (dim_im_out_y_no_pad - intermediate_rows); current_out_row+= intermediate_rows) {
    pulp_nn_depthwise_generic_HWC_HWC(pIn,
                                      Im2ColBuffer, 
                                      bias_dw, 
                                      intermediateBuffer, 
                                      weights_dw, 
  ...
  pulp_nn_pointwise_HoWo_parallel(
    ...
    pOut += intermediate_rows * dim_im_out_x * ch_im_out;
    pIn += (intermediate_rows * stride_y)  * dim_im_in_x * ch_im_in;
