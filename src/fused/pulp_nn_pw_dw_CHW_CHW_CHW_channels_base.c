...
void __attribute__ ((noinline)) pulp_nn_pw_dw_CHW_CHW_CHW_channels_base(
  ...
  for(; channels_done <= (ch_im_out - CHANNELS_BATCH_DW); channels_done += CHANNELS_BATCH_DW) {
    pulp_nn_pointwise_HoWo_parallel_CHW_CHW(
          /*pInBuffer=*/pIn,
          /*pIm2ColBuffer=*/Im2ColBuffer, // Needed because of CHW layout!
          /*bias=*/NULL,
    ...
    pulp_nn_depthwise_generic_CHW_CHW(
    ...
    pOut += (dim_im_out_x * dim_im_out_y) * CHANNELS_BATCH_DW;
  }
