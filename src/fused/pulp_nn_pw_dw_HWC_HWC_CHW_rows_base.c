...
void __attribute__ ((noinline)) pulp_nn_pw_dw_HWC_HWC_CHW_rows_base(
  ...
  uint16_t dim_im_out_y_no_pad = dim_im_out_y - padding_y_bottom; 
  ...
  if(output_rows< dim_im_out_y && core_id == 0) {
      memmove(intermediateBuffer, intermediateBuffer + (intermediate_rows_shift_in_padded * dim_im_in_x), intermediate_rows * dim_im_in_x * ch_im_out); 
  }
  ...
  pulp_nn_pointwise_HoWo_parallel_HWC_CHW(
        ...
        /*pIm2ColBuffer=*/NULL,
        /*bias=*/bias_pw,
        /*pOutBuffer=*/intermediateBuffer + (intermediate_rows - intermediate_rows_shift_in_padded) * dim_im_in_x,
        /*pWeight=*/weights_pw,
        /*k=*/k_pw,
        /*lambda=*/lambda_pw,
        /*out_mult=*/out_mult_pw,
  ...
  if(core_id == 0)
    memmove(intermediateBuffer, intermediateBuffer + (intermediate_rows_shift_in* dim_im_in_x), intermediate_rows * dim_im_in_x * ch_im_out); 
  pi_cl_team_barrier(0);
  ...
  if(current_out_row<dim_im_out_y) {
    ...
    int buffer_shift = (intermediate_rows - intermediate_rows_shift_in_padded) * dim_im_in_x;
  pulp_nn_pointwise_HoWo_parallel_HWC_CHW(
      /*pInBuffer=*/pIn,
      /*pIm2ColBuffer=*/NULL,
      /*bias=*/bias_pw,
      /*pOutBuffer=*/intermediateBuffer + buffer_shift,
      /*pWeight=*/weights_pw,
      /*k=*/k_pw,
      /*lambda=*/lambda_pw,
      /*out_mult=*/out_mult_pw,
